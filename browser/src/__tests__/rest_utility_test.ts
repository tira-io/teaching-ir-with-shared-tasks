...
test('Method get returns object from cache.', async () => {
    ...
    let request = {'path': 'path-does-not-exist', 'start': '1', 'end': '2'}
    let response = await get(request, obj)
...
test('Method get returns object from cache if multiple_keys_exist.', async () => {
    ...
    let request = {'path': 'path-does-not-exist', 'start': 1, 'end': 4}
    let response = await get(request, obj)
...
test('Method get returns against real http resource', async () => {
    ...
    let request = {'path': 'https://mam10eks.github.io/github-page-tutorial/run-details.jsonl', 'start': 108008, 'end': 109203}
    let response = await get(request, obj)
...
test('Error is thrown for wrong endpoints.', async () => {
    ...
    let request = {'path': 'https://mam10eks.github.io/github-page-tutorial/', 'start': 1000, 'end': 1002}
